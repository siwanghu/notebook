...
### 面向对象特性  
#### 封装  
> 将客观事物封装成抽象的类，并且类可以只将自己可信的属性和方法提供给外界操作，对不可信的进行信息隐藏  
#### 继承
> 继承提供子类使用父类的现有功能，并在无需重新编写原来父类的情况下对这些功能进行扩展
#### 多态  
> 类实例的相同方法在不同情形有不同的表现形式。具体来说就是不同实现类对公共接口有不同的实现方式，但这些操作可以通过相同的方式（公共接口）予以调用
### 面向对象设计原则  
#### 开-闭原则
+ 对扩展开放，对修改关闭
+ 设计要支持方便地扩展，同时要严格限制对已有内容的修改
#### 里氏替换原则
+ 子类必须能够替换其父类，否则不应当设计为其子类
+ 父类出现的地方，都应该能由其子类代替。子类只能去扩展基类，而不是隐藏或者覆盖基类
#### 依赖倒置原则
+ 设计和实现要依赖于抽象而非具体  
#### 接口隔离原则  
+ 将大的接口打散成多个小的独立的接口  
+ 尽量细化接口，接口中的方法尽量少,而不要试图去建立一个很庞大的接口供所有依赖它的类去调用
#### 单一职责原则  
+ 不要存在多于一个导致类变更的原因（高内聚低耦合）  
+ 类承担过多的职责，当由于其中一个职责需求发生改变而需要修改类时，有可能会导致原本运行正常的其它职责功能发生故障  
#### 最少知道原则
+ 一个对象尽可能少的去了解其它对象
#### 合成/聚合复用原则  
+ 新对象的某些功能在已经创建好的对象里面已经实现，那么应当尽量使用别的对象提供的功能，使之成为新对象的一部分，而不要再重新创建  
## 设计模式  
![设计模式](./pic/design.PNG)
## 创建型设计模式  
### 单例模式  
...
```  
### 简单工厂模式
> 在简单工厂模式中，可以根据参数的不同返回不同类的实例,简单工厂模式专门定义一个工厂类来负责创建其他类的实例，被创建的实例通常都具有共同的父类(接口)  
#### 实现方式
+ 定义所有产品都应该遵循的通用接口  
+ 工厂角色负责实现创建所有实例的内部逻辑，根据传入的参数(或在运行时根据定义的条件运算)创建并返回实例  
#### UML图  
![简单工厂模式](./pic/jdgc.png)  
#### 使用方式  
```
...
#### 实现方式
+ 所有产品都遵循同一接口,该接口必须声明对所有产品都有意义的方法  
+ 在抽象工厂类中声明工厂方法,用于返回一个产品,返回的类型必须是接口的形式,抽象工厂是工厂方法模式的核心，所有创建对象的工厂类都必须实现该接口  
+ 声明抽象工厂类的具体工厂子类，实现抽象工厂中定义的工厂方法，并可由客户端调用，返回一个具体产品类的实例
#### UML图
![工厂方法](./pic/factory_method.png)  
#### 使用方式  
```
...
#### 实现方式
+ 相同种类产品都遵循同一接口,该接口必须声明对相同种类产品都有意义的方法  
+ 在抽象工厂类中声明创建各个种类产品的工厂方法，所有创建对象的工厂类都必须实现这些方法  
+ 声明抽象工厂类的具体工厂子类，实现抽象工厂中定义的工厂方法，并可由客户端调用，返回具体产品类的实例
#### UML图
![抽象工厂](./pic/abstract_factory.png)  
#### 使用方式  
```
IFactory factory = new MySQLFactory();
IUserDao userDao = factory.createUserDao();
