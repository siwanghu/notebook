...
### 事务日志  
+ 以写入的第一个事务ZXID作为文件后缀名  
+ 事务日志大小固定为64MB  
+ 当写入事务日志的过程中,发现剩余可写入空间小于4KB时,进行扩容,新增事务日志文件（固定大小64MB）  
+ 文件空间预分配操作，避免随着每次事务的写入过程中文件大小增长带来的磁盘Seek开销
### 快照
+ 配置snapCount次事务操作触发数据快照  
+ 过半随机策略  
> 避免Zookeeper集群中所有机器在同一时刻进行数据快照,影响服务性能，采用过半随机的策略，判断是否需要进行数据快照**logCount > (snapCount / 2 + randRoll)   randRoll位1~snapCount / 2之间的随机数**
+ 利用异步线程进行数据快照避免影响Zookeeper主流程,以本次写入的第一个事务ZXID作为文件后缀名  
## 集群
![zk](./pic/zk)  
### 角色  
+ Leader  
